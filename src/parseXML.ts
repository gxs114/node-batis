...
function parseSqlNode (doc: Node) {
  ...
  const fragTags = content.filter((node: any) => node.type === "tag" && node.name === "fragment")
  const sqlTags = content.filter((node: any) => node.type === "tag" && node.name === "sql")
...
function parseSql (node: Node, fragmentMap: FragmentMap) {
  ...
  sql = sql.replace(/#\{\s*(.*?)\s*\}/g, (_, v: string) => `\${ __sql_escape( ${v} ) }`)
  return [
    node.attribs.id!,
    new Function(
      "query",
